...
export function PortfolioPageContent() {
  ...
  const { portfolio, projects, skillGroups, experiences, education, certifications, achievements } = content;
  const selectedProjects = projects.filter((project) => project.featured);
  const dialogueAriaPrefix = portfolio.ui.noteFrom;
  ...
  return (
    <>
      ...
      <section id="about" data-story-section="about" className="scroll-section section-divider py-14 sm:py-20">
        <Container>
          <SectionHeading eyebrow={portfolio.sections.about.eyebrow} title={portfolio.sections.about.title} description={portfolio.sections.about.body} />
          <SectionDialogue label={portfolio.scrollNarrative.label} message={portfolio.scrollNarrative.messages.about} ariaPrefix={dialogueAriaPrefix} />
      ...
      <section data-story-section="services" className="scroll-section py-14 sm:py-20">
        <Container>
          ...
          <SectionDialogue label={portfolio.scrollNarrative.label} message={portfolio.scrollNarrative.messages.services} ariaPrefix={dialogueAriaPrefix} />
      ...
      <section data-story-section="projects" className="scroll-section section-divider py-14 sm:py-20">
        <Container>
          <SectionHeading eyebrow={portfolio.sections.projects.eyebrow} title={portfolio.sections.projects.title} description={portfolio.sections.projects.description} />
          <SectionDialogue label={portfolio.scrollNarrative.label} message={portfolio.scrollNarrative.messages.projects} ariaPrefix={dialogueAriaPrefix} />
          <div className="reveal-grid mt-8 grid gap-5 md:grid-cols-2 xl:gap-6">
            {selectedProjects.map((project, index) => (
              <ProjectCard key={project.slug} project={project} index={index} systemLabel={portfolio.ui.projectSystemLabel} />
            ))}
          </div>
        </Container>
      </section>
      ...
      <section data-story-section="skills" className="scroll-section py-14 sm:py-20">
        <Container>
          ...
          <SectionDialogue label={portfolio.scrollNarrative.label} message={portfolio.scrollNarrative.messages.skills} ariaPrefix={dialogueAriaPrefix} />
      ...
      <section data-story-section="experience" className="scroll-section section-divider py-14 sm:py-20">
        <Container>
          ...
          <SectionDialogue label={portfolio.scrollNarrative.label} message={portfolio.scrollNarrative.messages.experience} ariaPrefix={dialogueAriaPrefix} />
      ...
      <section data-story-section="credentials" className="scroll-section py-14 sm:py-20">
        <Container>
          ...
          <SectionDialogue label={portfolio.scrollNarrative.label} message={portfolio.scrollNarrative.messages.credentials} ariaPrefix={dialogueAriaPrefix} />
          <div className="reveal-grid mt-8 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
            <div className="site-card reveal-card rounded-[1.75rem] p-6 sm:p-8">
              <p className="eyebrow">{portfolio.ui.educationLabel}</p>
              {education.map((item) => (
              ...
              <div className="mt-8 border-t border-[var(--border)] pt-7">
                <p className="eyebrow">{portfolio.ui.certificationsLabel}</p>
            ...
            <div className="site-card reveal-card rounded-[1.75rem] p-6 sm:p-8">
              <p className="eyebrow">{portfolio.ui.achievementsLabel}</p>
      ...
      <section data-story-section="contact" className="scroll-section pb-16 pt-6 sm:pb-24 sm:pt-10">
        <Container>
          <div className="cta-panel reveal-card rounded-[2rem] p-7 sm:p-10 lg:p-12">
            ...
            <SectionDialogue label={portfolio.scrollNarrative.label} message={portfolio.scrollNarrative.messages.contact} ariaPrefix={dialogueAriaPrefix} />
            <h2 className="mt-4 max-w-4xl text-3xl font-semibold tracking-[-0.035em] text-main sm:text-4xl lg:text-5xl">{portfolio.sections.contact.title}</h2>
            <p className="mt-5 max-w-3xl text-base leading-8 text-muted">{portfolio.sections.contact.body}</p>
            <div className="mt-8 border-t border-[var(--border)] pt-6">
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-subtle">{portfolio.ui.emailLabel}</p>
